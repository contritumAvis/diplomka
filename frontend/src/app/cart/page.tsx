...
export default function CartPage() {
  ...
  const fetchCart = async () => {
    ...
    try {
      ...
      console.log('Cart data:', data); // проверка что приходит
                ...
                >
                  ...
                  <div className="flex items-center flex-1 gap-4">
                    {item.product?.thumbnail ? (
                      ...
                      <div className="w-[72px] h-[72px] bg-gray-200 rounded-sm flex items-center justify-center text-gray-500 text-[12px]">
                        Нет изображения
                      </div>
                    )}
                    <span className="text-gray-700 text-[14px] font-normal">
                      {item.product?.name || 'Продукт не найден'}
                    </span>
                  </div>

                  {/* Цена */}
                  <div className="w-[200px] text-gray-900 text-[14px] font-medium">
                    {item.product?.basePrice ? `${item.product.basePrice}₸` : '-'}
                  </div>
